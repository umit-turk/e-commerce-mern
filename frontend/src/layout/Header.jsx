        ...
        <div className="bg-gray-100 h-16 px-5 flex justify-between items-center">
            ...
            <div className="flex items-center gap-5">
                ...
                <div className="relative">
                    <img onClick={() => setOpenMenu(!openMenu)} className="w-8 h-8 rounded-full cursor-pointer" src={user?.user?.avatar?.url ? user?.user?.avatar?.url : "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQQLHZh0aF5Og2DF4G19yPVx_QGjXfaBByFZA&usqp=CAU"} alt="" />
                 {openMenu &&  <div className="absolute right-0 mt-3 w-[200px] bg-white shadow-lg shadow-gray-100">
                        {menuItems.map((item, i) => (
                            <div onClick={() => navigate(`${item?.url}`)} className="px-2 py-1 cursor-pointer hover:bg-gray-100" key={i}>{item.name}</div>
                        ))}
                    </div>}
                </div>
                <div className="relative">
